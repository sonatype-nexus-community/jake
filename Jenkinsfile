  ...
  vulnerabilityScan: {
    withDockerImage(env.DOCKER_IMAGE_ID, {
      withCredentials([usernamePassword(credentialsId: 'policy.s integration account',
        ...
        poetry run jake iq --application jake --stage stage-release --username $IQ_USERNAME --password $IQ_PASSWORD --server-url https://policy.ci.sonatype.dev        
        #poetry run jake iq --application jake --stage stage-release --username $IQ_USERNAME --password $IQ_PASSWORD --server-url https://policy.ci.sonatype.dev || export jakeFailed=true        
